...
import { ParameterDataTable } from '../components/parameter-data-table'
import { ParameterFilterBar } from '../components/parameter-filter-bar'
import { ParameterFormContainer } from '../components/parameter-form-container'
import { ParameterDeleteDialog } from '../components/parameter-delete-dialog'
...
export function ParameterPage() {
  ...
  const [deleteRecord, setDeleteRecord] = useState<ParameterType | null>(null)
  const { openModal } = useModalStore()

  const filter = useParameterFilterStore()
  const { data, isLoading } = useGetParameters({ filter })
  const deleteMutation = useDeleteParameter()

  // Handle edit parameter
  const handleEditParameter = (parameter: ParameterType) => {
    openModal(
      { id: parameter.id },
      `Edit Parameter: ${parameter.parameter_name}`
    )
  }

  // Handle delete parameter
  const handleDeleteParameter = (parameter: ParameterType) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(
        `Parameter ${deleteRecord.parameter_name} deleted successfully`
      )
  ...
  const handleCreateNew = () => {
    openModal(null, 'Create New Parameter')
  }
  ...
  return (
    <div className="w-full mx-auto p-4 md:p-6 lg:p-8">
      <Card className="shadow-none border-none">
        <PageHeader
          title="Parameters"
          description="Manage and view all testing parameters for quality control"
          actions={headerActions}
        />
        <div className="p-0 pb-6 md:pb-2 pt-0 md:pt-2 md:px-2">
          <ParameterFilterBar showFiltersButton={false} />
        </div>
      ...
      {/* Create/Edit Modal */}
      <ParameterFormContainer onSuccess={handleFormSuccess} />
    </div>
  )
}
