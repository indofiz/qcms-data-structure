...
import { useGetAllRawMaterials } from '@/features/master-data/raw-material/hooks/use-raw-materials'
import { useEffect, useState } from 'react'

interface CoaStandardizationFilterBarProps {
  showFiltersButton?: boolean
}

export function CoaStandardizationFilterBar({
  showFiltersButton = true,
}: CoaStandardizationFilterBarProps) {
  const {
    search,
    raw_material,
    setSearch,
    setRawMaterial,
    resetFilter,
  } = useCoaStandardizationFilterStore()

  // Fetch raw materials data
  const { data: rawMaterialsData, isLoading: rawMaterialsLoading } =
  ...
  const activeFiltersCount = [raw_material].filter(Boolean).length
    ...
    <div className="space-y-4">
      ...
      >
        <div className="relative flex-1 w-full">
          ...
          <Input
            placeholder="Search COA standardizations..."
            value={search}
      ...
      {showFiltersButton && showFilters && (
        <div className="flex flex-wrap items-center gap-3 p-4 bg-muted/30 rounded-lg border animate-in slide-in-from-top-2 duration-200">
          ...
          {/* Raw Material Filter */}
          <Select
            value={raw_material || 'all'}
            onValueChange={(value) =>
              setRawMaterial(value === 'all' ? undefined : value)
            }
          >
            ...
            <SelectContent>
              ...
              {rawMaterialsLoading ? (
                ...
                rawMaterialsData?.data?.data?.map((material) => (
